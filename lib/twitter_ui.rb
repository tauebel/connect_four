...
class TwitterUI
  ...
  def find_challenger
    ...
    TweetStream::Client.new.track("#{@handle} Game on! #dbc_c4") do |status|
      ...
      @player1 = @handle
      @player2 = "@#{status.user.screen_name}"
      break if @player2 != nil
    end
    @home = true
    play_game
  end
  ...
  def accept_challenge
    ...
    TweetStream::Client.new.track("Who wants to get demolished? #dbc_c4") do |status|
      @player1 = status.user.screen_name if status.user.screen_name !="longfourplay"
      break if @player1 != nil
    end
    @player2 = @handle
    @home = false
    play_game
...
def send_board
    ...
    @home == true ? @opponent = @player2.name : @opponent = @player1.name
    Twitter.update("@#{opponent} #{@twitter_board} #dbc_c4 ##{rand(1000)}")
end
...
def end_game
  ...
  puts last_message
  Twitter.update("@#{@opponent} #{last_message} #dbc_c4 ##{rand(1000)}")
  return
end
...
def setup_game
  @player1 = Player.new(@player1, "#{rand(100000)}@twitter.com")
  @player2 = Player.new(@player2, "#{rand(100000)}@twitter.com")
...
TwitterUI.new("@jakesendar", TwitterAuth.new).accept_challenge if command[0] = "home"
TwitterUI.new("@jakesendar", TwitterAuth.new).find_challenger if command[0] = "away"

# twitter_ui = TwitterUI.new("@jakesendar", TwitterAuth.new)
# twitter_ui.accept_challenge
#twitter_ui.find_challenger
