require_relative 'twitter_slot'

class TwitterBoard
  attr_reader :slots, :available_slots, :fields, :board_string
  def initialize
    @full = false
  ...
  def board_string
    twitter_board = []
    @slots.each do |row|
      twitter_board << row.join.to_s + "|"
    end
    twitter_board << "|"
    twitter_board.reverse.join.to_s
  end
...
private
  ...
  def generate_slots
    [[],[],[],[],[],[]].each {|row| 7.times {row << TwitterSlot.new } }
  end
...
#print TwitterBoard.new.board_string
